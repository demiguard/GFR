...
class RisFetcher():
  ...
  def associate(self, department : Department):
    ...
    self.ris_assoc = ae_controller.establish_assoc(
      ae_controller.create_find_AE(department.ris_calling),
  ...
  def move_dataset(self, historic_dataset : Dataset, dataset_dir : Path):
    target_file = Path(f"{server_config.SEARCH_DIR}{historic_dataset.accession_number}.dcm")
    destination = Path(f"{dataset_dir}{historic_dataset.accession_number}.dcm")
  ...
  def fetch_history(self, dataset: Dataset, dataset_dir : Path) -> None:
    ...
    response = self.pacs_find_assoc.send_c_find(history_queryDataset, StudStudyRootQueryRetrieveInformationModelFind)
